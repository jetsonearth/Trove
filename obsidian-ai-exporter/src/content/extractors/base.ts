...
export abstract class BaseExtractor implements IConversationExtractor {
  ...
  async extract(): Promise<ExtractionResult> {
    try {
      ...
      // Fetch real conversation date from platform API (best-effort)
      let conversationDate: Date | null = null;
      try {
        conversationDate = await this.getConversationDate();
      } catch (e) {
        console.warn(`[G2O] Could not fetch conversation date:`, e);
      }

      const result = this.buildConversationResult(messages, conversationId, title, this.platform, conversationDate);
  ...
  /**
   * Fetch the conversation's actual creation date from the platform API.
   * Override in subclasses to make same-origin API calls.
   * Returns null by default (falls back to extraction time).
   */
